...
# ## 배열의 종류: 사전
{어 나야. 사전(Dictionary). 사전은 인덱스 아니고 key + value로 구성돼 있대. 뭔말이야.}
# In[1164]:
...
print(age['동혁'])
print(age.get('동혁')) #2가지 방법이 있다.

print('동혁 나이는', age.get('동혁') , '\b세입니다')
...
22.05.11) 오늘도 내 힘으로 푼 거 아니다, ㅋ,
앞으론 웬만하면 수업 시간에 나가지 말아야겠다. 
# In[40]:
...
# ## 배열의 종류: 집합 (p.107)
집합은 사전이랑 비슷하지만, key만 있고 value는 없다.
key들만 순서 없이 아무렇게나 막 들어가 있는 놈이다.

새로운 거 넣을 때도 순서 없이 막 들어간다...
# In[41]:
...
이 외에도 remove(해당 원소), pop(), clear()도 가능.
# In[45]:


#Checking membership도 가능.

fruits = {'사과', '오렌지', '포도', '수박'}
print('사과' in fruits)


# ### 집합의 연산 - 퀴즈 5.21, 5.22(p.109)
...
word = 'Python Programming' #공백 포함, 18개 문자의 문자형 배열
letters = set(word)
print(len(letters)) #이럭개 답이 나온 이유: 1.집합에선 공백도 한 문자로 ㅇㅈ해 준다. 2.그러나 집합에선 중복을 배제한다. (아 참고로 P랑 p는 다른 글자다. 아스키 코드도 다르잖아.)


# ## 인공지능을 위한 빅데이터 분석의 예제 어쩌고 (p.110)
코드와 결과를 잘 읽고, 대충 코드가 뭔 쌉행동을 하는 놈인지만 잘 이해하면 된다.
할 수 있! 🥰

(그나저나 2차원 배열을 함수 입력변수로 입력할 수 있다는 점, 2차원 배열은 for row in data: 식으로 for문 돌리면 한 줄씩 읽힌다는 점 등등이 주목할 만 하다, ㅋ.)
# In[52]:
...
def fruit_counts(data):
    counts = {} #Set이 아니고 Dictionary 형식이다.
    
    for row in data:
        label = row[-1]
        
        if label not in counts:
            counts[label] = 0 #이 줄이랑 출력 결과를 봐라. Set이 아니고 Dictionary잖. (그러게. 키랑 밸류가 있내.)
        counts[label] += 1
...
아, 그리고 늘 생각하는 건데, 💥변수 이름은 꼭 정해진 게 아니다💥.
함수는 단지 뭐가 어디에 있냐로만 작동 원리가 정해진다. 그걸 체화하세요.

(그렇지만 아래처럼 변수 이름을 이상하게 지으니까, 확실히 가독성이 떨어지긴 하네.)
# In[57]:
...
def fruit_counts(data):
    ...
    for bruh in data: #이름을 어떻게 짓든 간에, 2차원 배열은 한 줄씩 읽는디.
        gangsta = bruh[0]
        
        if gangsta not in pocket:
            pocket[gangsta] = 0     
        pocket[gangsta] += 1
